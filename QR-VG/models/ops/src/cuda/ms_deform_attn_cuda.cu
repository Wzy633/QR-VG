    ...
    const at::Tensor &pruning_mask)
{
    ...
    const bool use_pruning_mask = pruning_mask.defined() && pruning_mask.numel() > 0;
    if (use_pruning_mask)
    {
        AT_ASSERTM(pruning_mask.is_contiguous(), "pruning_mask tensor has to be contiguous");
        AT_ASSERTM(pruning_mask.is_cuda(), "pruning_mask must be a CUDA tensor");
    }
    ...
    for (int n = 0; n < batch/im2col_step_; ++n)
    {
        ...
        AT_DISPATCH_FLOATING_TYPES(value.scalar_type(), "ms_deform_attn_forward_cuda", ([&] {
            const bool* pruning_mask_ptr = use_pruning_mask ? pruning_mask.data<bool>() + n * im2col_step_ * spatial_size : nullptr;
