...
def inverse_sigmoid(x, eps=1e-5):
    """
    数值稳定的inverse_sigmoid实现
    
    改进：
    1. 更严格的输入限制：确保x在[eps, 1-eps]范围内
    2. 防止log溢出：限制x1/x2的范围
    """
    x = x.clamp(min=eps, max=1-eps)
    x1 = x
    x2 = 1 - x
    ratio = x1 / x2
    ratio = ratio.clamp(min=1e-10, max=1e10)
    result = torch.log(ratio)
    result = result.clamp(min=-10.0, max=10.0)
    return result


class WarmupMultiStepLR:
    """
    带Warmup的MultiStepLR调度器
    可以直接替换现有的lr_scheduler，兼容checkpoint保存/加载
    """
    def __init__(self, optimizer, milestones, gamma=0.1, warmup_epochs=0, warmup_factor=0.01):
        """
        Args:
            optimizer: 优化器
            milestones: MultiStepLR的milestones（相对于总epoch）
            gamma: 学习率衰减因子
            warmup_epochs: warmup的epoch数（0表示不使用warmup）
            warmup_factor: warmup起始学习率 = warmup_factor * base_lr（默认0.01，即从1%开始）
        """
    ...
    def step(self, epoch=None):
        ...
        if self.warmup_epochs > 0 and epoch < self.warmup_epochs:
            warmup_lr = [
                base_lr * (self.warmup_factor + (1 - self.warmup_factor) * (epoch + 1) / self.warmup_epochs)
                for base_lr in self.base_lrs
            ]
            for param_group, lr in zip(self.optimizer.param_groups, warmup_lr):
                param_group['lr'] = lr
        else:
            num_reductions = sum(1 for m in self.milestones if epoch >= m)
            for param_group, base_lr in zip(self.optimizer.param_groups, self.base_lrs):
                param_group['lr'] = base_lr * (self.gamma ** num_reductions)
    ...
    def load_state_dict(self, state_dict):
        ...
        self.warmup_factor = state_dict.get('warmup_factor', self.warmup_factor)
